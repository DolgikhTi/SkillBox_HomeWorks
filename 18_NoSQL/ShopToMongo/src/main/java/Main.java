...
public class Main {
    ...
    public static void setItem(String itemName, String shopName){
        ...
        try {
            if (!(collection_item.find(queryItem).first() == null) && !(collection_shop.find(queryShop).first() == null))
            {
                ...
                {
                    collection_shop.findOneAndUpdate(queryShop,
                            BsonDocument.parse("{$addToSet: {itemList: \"" + itemName + "\"}}"));
                }
                else {
                    collection_shop.findOneAndUpdate(queryShop,
                            BsonDocument.parse("{$set: {itemList: [\"" + itemName + "\"]}}"));
                }
            }
            else {
                System.out.println("магазин: " + collection_shop.find(queryShop).first() +
                        ", товар: " + collection_item.find(queryItem).first());
    ...
    public static void showStatistics(){
        ...
        collection_shop.aggregate(Arrays.asList(unwind("$itemList"),
                sortByCount("$name"))).forEach((Consumer<Document>) document -> {
            System.out.println("Наименование магазина: " + document.get("_id") + " - количество товара: "
                    + document.get("count"));
        });
        ...
        )).forEach((Consumer<Document>) document -> System.out.println("Наименование магазина: " + document.get("_id") +
                " - Средняя цена товара: " + document.get("avgPriceItem")));
        ...
        collection_shop.aggregate(Arrays.asList(
                ...
                System.out.println("Наименование магазина: " + document.get("_id") + "\n\t " +
                        "- Самый дешевый товар: " + document.get("minNameItem") + " - "
                        + document.get("minPriceItem") + " руб." + "\n\t " +
                        "- Самый дорогой товар: " + document.get("maxNameItem") + " - "
                        + document.get("maxPriceItem") + " руб."));
